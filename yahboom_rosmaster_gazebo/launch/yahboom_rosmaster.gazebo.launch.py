...
def generate_launch_description():
    ...
    declare_gz_world_name_cmd = DeclareLaunchArgument(
        name='gz_world_name',
        default_value='factory_world',
        description='Gazebo internal world name (matches <world name=...> in .world file)')

    declare_use_gz_pose_tf_cmd = DeclareLaunchArgument(
        name='use_gz_pose_tf',
        default_value='true',
        choices=['true', 'false'],
        description='Publish map→odom TF from Gazebo ground truth. Set false when using SLAM.')
    ...
    # Publish map→odom correction TF from Gazebo ground truth (disable when using SLAM)
    gz_msgs_python_path = '/opt/ros/jazzy/opt/gz_msgs_vendor/lib/python'
    existing_pythonpath = os.environ.get('PYTHONPATH', '')
    gz_pythonpath = (
        gz_msgs_python_path + ':' + existing_pythonpath
        if existing_pythonpath else gz_msgs_python_path
    )
    start_gz_pose_tf_publisher_cmd = Node(
        package='waypoint_follower',
        executable='gz_pose_tf_publisher',
        name='gz_pose_tf_publisher',
        output='screen',
        parameters=[{
            'use_sim_time': use_sim_time,
        ...
        additional_env={'PYTHONPATH': gz_pythonpath},
        condition=IfCondition(LaunchConfiguration('use_gz_pose_tf')))
    ...
    ld.add_action(declare_gz_world_name_cmd)
    ld.add_action(declare_use_gz_pose_tf_cmd)
    ...
    ld.add_action(start_gz_pose_tf_publisher_cmd)
