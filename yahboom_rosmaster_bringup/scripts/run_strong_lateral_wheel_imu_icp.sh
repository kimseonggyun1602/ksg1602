#!/bin/bash
# Run SLAM with degraded wheel odometry, IMU and RTAB-Map ICP LiDAR odometry.
...
RESULTS_DIR="${2:-$HOME/ksg_results/strong_lateral_wheel_imu_icp}"
...
if ros2 topic info /clock 2>/dev/null | grep -q 'Publisher count: [1-9]'; then
    ...
    echo "Stop old launches before starting the wheel, IMU and ICP run." >&2
    exit 1
fi
...
echo "============================================================"
echo "Strong lateral wheel, IMU and ICP LiDAR odometry SLAM"
echo "Bag:     $BAG_PATH"
...
echo "EKF input 1:            /imu/data yaw rate"
echo "EKF input 2:            /icp/odom"
echo "EKF TF output:          odom -> base_footprint"
echo "============================================================"

setsid ros2 launch waypoint_follower replay_wheel_imu_icp_slam.launch.py \
    bag_path:="$BAG_PATH" \
...
echo
echo "============================================================"
echo "Strong lateral wheel, IMU and ICP run completed"
...
if [ "$SHOW_RESULT_RVIZ" = "true" ]; then
    ...
    exec ros2 launch waypoint_follower \
        view_strong_lateral_wheel_imu_icp_result.launch.py \
        results_dir:="$RESULTS_DIR"
fi
