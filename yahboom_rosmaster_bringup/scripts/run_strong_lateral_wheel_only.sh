#!/bin/bash
# Run wheel-only SLAM with lateral-motion degradation and deterministic slip events.
...
RESULTS_DIR="${2:-$HOME/ksg_results/strong_lateral_wheel_only}"
...
if ros2 topic info /clock 2>/dev/null | grep -q 'Publisher count: [1-9]'; then
    ...
    echo "Stop old launches before starting the strong wheel-only run." >&2
    exit 1
fi
...
echo "============================================================"
echo "Strong lateral wheel-only SLAM"
echo "Bag:     $BAG_PATH"
...
echo
echo "============================================================"
echo "Strong lateral wheel-only run completed"
...
if [ "$SHOW_RESULT_RVIZ" = "true" ]; then
    ...
    exec ros2 launch waypoint_follower view_strong_lateral_result.launch.py \
        results_dir:="$RESULTS_DIR"
fi
