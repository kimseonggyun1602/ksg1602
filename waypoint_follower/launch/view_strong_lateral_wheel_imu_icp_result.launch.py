...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            name='saved_strong_wheel_imu_icp_result_viewer',
            output='screen',
            parameters=[{
                'reference_yaml': LaunchConfiguration('reference_yaml'),
                'gt_csv': LaunchConfiguration('gt_csv'),
                'results_dir': LaunchConfiguration('results_dir'),
                'result_id': 'wheel_imu_icp',
            }],
        ),
        Node(
            package='rviz2',
            executable='rviz2',
            name='rviz_saved_strong_wheel_imu_icp_result_viewer',
            output='screen',
            arguments=[
                '-d',
                os.path.join(
                    share, 'rviz',
                    'strong_wheel_imu_icp_saved_result_viewer.rviz'),
            ],
        ),
    ])
